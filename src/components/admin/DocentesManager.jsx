...
export function DocentesManager() {
  ...
  async function handleResetPassword(doc) {
    if (!doc.user_id) {
      toast.error('Este docente no tiene una cuenta de acceso.')
      return
    }
    if (!confirm(
      `¿Restablecer la contraseña de ${doc.nombre_completo} a "${PASSWORD_POR_DEFECTO}"?\n\n` +
      `Deberá crear una contraseña nueva la próxima vez que inicie sesión.`
    )) return

    setReseteandoId(doc.id)
    const { ok, error } = await resetearPassword(doc.user_id)
  ...
  const docentesFiltrados = termino
    ? docentes.filter(d =>
        [d.nombre_completo, d.email, d.numero_documento, d.telefono]
                ...
                return (
                  <tr key={doc.id} className="border-t border-[#e8dcca] hover:bg-[#f5efe6] transition">
                    ...
                    <td className="px-4 py-3 text-center whitespace-nowrap">
                      ...
                      >
                        {reseteandoId === doc.id ? '⏳' : '🔑'} Restablecer
                      </button>
                      <button
